...
date: 'This version: December 2025'
author: "Adeline Lo"
output: 
  html_document: 
    code_folding: hide
...
studies<-data.frame(Study=c("Study 1","Study 2", "Study 3", "Study 4","Study 5A","Study 5B", "Study 6", "Study 7", "Total")
                    ,Goal=c("Costliness of empathy","Eliciting peer praise","Peer praise on empathy","Happiness as mediator","Mediation analysis","Mediation analysis","Empathy towards racial outgroups","Co-partisan praise","-")
                    ,Total_N=c(nrow(data1),nrow(data2),nrow(data3),nrow(data4),nrow(data5),nrow(data6),214,308,sum(c(nrow(data1),nrow(data2),nrow(data3),nrow(data4),nrow(data5),nrow(data6),214,308)))
                    ,Total_T=c(3,"-",15,1,20,3,1,1,"-")
                    ,Total_obs=c(nrow(long_data1),nrow(data2) #1,2
                                 ,(1559+1561+1560),nrow(data4) #3,4
                                 ,(3238+3242),(866+865),214,308
                                 ,sum(c(nrow(long_data1),nrow(data2),(1559+1561+1560),nrow(data4),(3238+3242),(866+865),214,308)))
)
...
# Fit model explicitly (so we can report the test + exact p-value)
fit <- lm(thermometer ~ positive, data = sent_feel_df)
s <- summary(fit)
# Exact p-value for the slope (t-test of H0: beta_positive = 0)
p_slope <- s$coefficients["positive", "Pr(>|t|)"]
beta    <- s$coefficients["positive", "Estimate"]
se_beta <- s$coefficients["positive", "Std. Error"]
df_res  <- s$df[2]
# Format EXACT p-value (not thresholds)
p_txt <- format.pval(p_slope, digits = 3, eps = .Machine$double.eps)
# Annotation text (specifies test + exact p-value)
stat_lab <- sprintf("OLS: N=115, slope = %.3f, p = %s", beta,  p_txt)
# Plot
tmp_col <- viridis::viridis(3, begin = 0, end = 1, direction = 1, option = "D", alpha = 0.65)
p1 <- ggplot(sent_feel_df, aes(x = positive, y = thermometer)) +
  coord_cartesian(ylim = c(0, 10)) +
  geom_point(
    position = position_jitter(width = 0.2, height = 0.2, seed = 12345),
    alpha = 0.5
  ) +
...
# Fit model explicitly
fit2 <- lm(thermometer ~ negative, data = sent_feel_df)

# Exact p-value for slope (negative)
p2_slope_p <- summary(fit2)$coefficients["negative", "Pr(>|t|)"]
p2_slope_est <- coef(fit2)["negative"]

# Format for display (exact, but readable)
p2_p_txt <- ifelse(p2_slope_p < 1e-3,
                   format(p2_slope_p, scientific = TRUE, digits = 2),
                   formatC(p2_slope_p, format = "f", digits = 4))
p2_b_txt <- formatC(p2_slope_est, format = "f", digits = 3)

# Make plot with annotation
p2 <- ggplot(sent_feel_df, aes(x = negative, y = thermometer)) +
  ylim(0, 10) +
  geom_point(position = position_jitter(seed = 12345),
             alpha = 0.5, width = 0.2, height = 0.2) +
  geom_smooth(method = lm, color = cbPalette[1], fill = tmp_col[3], se = TRUE) +
  annotate(
    "text",
    x = -Inf, y = Inf, hjust = -0.05, vjust = 1.15,
    label = paste0("OLS: N=115, ",
                   "slope = ", p2_b_txt, ", p = ", p2_p_txt),
    size = 3.5 #
  ) +
  theme_bw()
...
#How are text sentiments and the thermometer related?
# Fit model explicitly (so we can report the test + exact p-value)
fit <- lm(thermometer ~ positive, data = sent_describe_df)
s <- summary(fit)
# Exact p-value for the slope (t-test of H0: beta_positive = 0)
p_slope <- s$coefficients["positive", "Pr(>|t|)"]
beta    <- s$coefficients["positive", "Estimate"]
...
# Plot
tmp_col <- viridis::viridis(3, begin = 0, end = 1, direction = 1, option = "D", alpha = 0.65)
p1 <- ggplot(sent_describe_df, aes(x = positive, y = thermometer)) +
  coord_cartesian(ylim = c(0, 10)) +
  geom_point(
    position = position_jitter(width = 0.2, height = 0.2, seed = 12345),
    alpha = 0.5
...
#negative sentiment
# Fit model explicitly (so we can report the test + exact p-value)
fit <- lm(thermometer ~ negative, data = sent_describe_df)
s <- summary(fit)
# Exact p-value for the slope (t-test of H0: beta_positive = 0)
p_slope <- s$coefficients["negative", "Pr(>|t|)"]
beta    <- s$coefficients["negative", "Estimate"]
# Format EXACT p-value (not thresholds)
p_txt <- format.pval(p_slope, digits = 3, eps = .Machine$double.eps)
# Annotation text (specifies test + exact p-value)
stat_lab <- sprintf("OLS: N=115, slope = %.3f, p = %s", beta,  p_txt)
# Plot
tmp_col <- viridis::viridis(3, begin = 0, end = 1, direction = 1, option = "D", alpha = 0.65)
p2 <- ggplot(sent_describe_df, aes(x = negative, y = thermometer)) +
  coord_cartesian(ylim = c(0, 10)) +
  geom_point(position = position_jitter(width = 0.2, height = 0.2, seed = 12345), alpha = 0.5) +
  geom_smooth(method = "lm", color = cbPalette[1], fill = tmp_col[2], se = TRUE) +
  annotate("label",
    x = -Inf, y = Inf,
    hjust = -0.05, vjust = 1.1,
...
n_feel     <- sum(!is.na(plotdata$feel))
n_describe <- sum(!is.na(plotdata$describe))
test_res <- t.test(plotdata$feel, plotdata$describe, paired = FALSE)
p_exact <- test_res$p.value
p_txt <- ifelse(p_exact < 1e-3,
                format(p_exact, scientific = TRUE, digits = 2),
                formatC(p_exact, format = "f", digits = 4))
mean_feel     <- mean(plotdata$feel, na.rm = TRUE)
mean_describe <- mean(plotdata$describe, na.rm = TRUE)

p <- ggplot(plotdata) +
  # Feel (top)
  geom_histogram(aes(x = feel, y = after_stat(density)),
                 fill = "gold2", binwidth = 0.5, alpha = 0.7) +

  # CHANGED (Nature): include n in the figure label, and keep “Average …”
  geom_label(aes(x = 4.5, y = 0.35,
                 label = paste0("Average Feel: ", sprintf("%.2f", mean_feel),
                                "\n(n = ", n_feel, " samples)")),
             color = "gold2") +

  # Describe (bottom, mirrored)
  geom_histogram(aes(x = describe, y = -after_stat(density)),
                 fill = "darkcyan", binwidth = 0.5, alpha = 0.7) +

  # CHANGED (Nature): include n in the figure label
  geom_label(aes(x = 4.5, y = -0.35,
                 label = paste0("Average Describe: ", sprintf("%.2f", mean_describe),
                                "\n(n = ", n_describe, " samples)")),
             color = "darkcyan") +

  # CHANGED (Nature): put test + exact p-value ON the figure (or you could put in caption)
  annotate("text", x = 4.5, y = 0.52,
           label = paste0("Two-sample t-test (two-sided): p = ", p_txt),
           size = 3.6) +
...
df <- data4 %>%
  filter(!is.na(PraiseEmpathy), !is.na(pride)) %>%      # <-- CHANGED
  mutate(name = ifelse(PraiseEmpathy == 1,
                       "Peer praise empathy",
                       ifelse(PraiseEmpathy == 0, "Control", NA_character_)))

tt <- t.test(pride ~ PraiseEmpathy, data = df) 
p_exact <- tt$p.value
p_txt <- ifelse(p_exact < 1e-3,
                format(p_exact, scientific = TRUE, digits = 2),
                formatC(p_exact, format = "f", digits = 4))
diff_means <- diff(tt$estimate)
diff_txt <- paste0("N = ", sprintf("%.0f", nrow(df)),", Difference in means: ",  sprintf("%.3f", diff_means),
                   " (two-sample t-test p = ", p_txt, ")")
...
# Plot
praiseprideplot <- ggplot(df, aes(x = name, y = pride, fill = name)) +
  geom_violin(width = 1, trim = FALSE) +
  geom_boxplot(width = 0.1, color = "grey20",
               alpha = 0.2, outlier.shape = NA) +
  # CHANGED (Nature: overlay individual data points)
  geom_point(
    position = position_jitter(width = 0.12, height = 0, seed = 12345),
    alpha = 0.55, size = 1.6, color = "grey20"
  ) +
  scale_fill_viridis(discrete = TRUE, alpha = 0.7,
                     begin = 0.25, end = 1, option = "D") +
  theme_bw() +
  ylab("Pride") + xlab("") +
  ggtitle("Pride distribution by treatment arm") +
  theme(
  ...
  coord_flip() +
  # CHANGED (Nature: test + exact p-value on figure)
  annotate(
    "text",
    x = 1.5, y = 2,
    label = diff_txt,
...
ggsave("Figures/praise-pride_dist.pdf",  width = 11, height = 7)
#Figures/praise-pride_dist.pdf 11x7
```
...
summary(model1_1)
n_model <- nobs(model1_1)  # 204 exact n used by glm after dropping NAs
coefs <- summary(model1_1)$coefficients
p_exact <- coefs["PraiseEmpathy", "Pr(>|z|)"]   # Wald test p-value
...
stat_txt <- paste0(
  "Logit coefficient on Peer Praise: ", sprintf("%.3f", coefs[2,1]),
  " (p = ", p_txt, "), N=",n_model
)

# Put model estimates into temporary data.frames:
main_variable_names<-c("Intercept","Peer Praise")
...
#caption: \textbf{Pilot Study 6 main effect: Peer-praise effect on outgroup bias for attentive white respondents, trial 1.} Points plotted at the center of bands are logistic regression coefficients of control (intercept = -1.073) and Peer Praise (=0.756) of choosing empathy for the outgroup regressed on Peer Praise (N=204). Bands are (thick) 90\% and (thin) 95\% confidence intervals. The baseline odds of choosing FEEL over DESCRIBE is 0.34 times; the odds of choosing FEEL over DESCRIBE if respondents are in the treatment group is 2.13 (p=0.0195\% CI [1.18, 3.89]) compared to the control group.}
```
...
model1a<-lm_robust(describeORfeel~CoParty_control,data7)%>% tidy %>% 
  mutate(.,
         outcome = "Choice Task",
         Sample = "Full") 
modelsummary<-summary(lm_robust(describeORfeel~CoParty_control,data7))
n_model <- nobs(lm_robust(describeORfeel~CoParty_control,data7)) #267
coefs <- modelsummary$coefficients
p_exact <- coefs["CoParty_control", "Pr(>|t|)"]   #

#text for Choice-Full model
p_txt <- ifelse(p_exact < 1e-3,
                format(p_exact, scientific = TRUE, digits = 2),
                formatC(p_exact, format = "f", digits = 4))

stat_txt_choicefull <- paste0(
  "OLS coefficient Co-Partisan Peer Praise:\n ", sprintf("%.3f", coefs[2,1]),
  " (p = ", p_txt, "), N=",n_model
)
...
modelsummary<-summary(lm_robust(describeORfeel~CoParty_control,data7_attentive))
n_model <- nobs(lm_robust(describeORfeel~CoParty_control,data7_attentive)) #100
coefs <- modelsummary$coefficients
p_exact <- coefs["CoParty_control", "Pr(>|t|)"]   #

#text for Choice-Attentive model
p_txt <- ifelse(p_exact < 1e-3,
                format(p_exact, scientific = TRUE, digits = 2),
                formatC(p_exact, format = "f", digits = 4))

stat_txt_choiceattentive <- paste0(
  "OLS coefficient Co-Partisan Peer Praise:\n ", sprintf("%.3f", coefs[2,1]),
  " (p = ", p_txt, "), N=",n_model
)
...
modelsummary<-summary(lm_robust(BLM~CoParty_control,data7))
n_model <- nobs(lm_robust(BLM~CoParty_control,data7)) #249
coefs <- modelsummary$coefficients
p_exact <- coefs["CoParty_control", "Pr(>|t|)"]   #

#text for BLM-Full model
p_txt <- ifelse(p_exact < 1e-3,
                format(p_exact, scientific = TRUE, digits = 2),
                formatC(p_exact, format = "f", digits = 4))

stat_txt_blmfull <- paste0(
  "OLS coefficient Co-Partisan Peer Praise:\n ", sprintf("%.3f", coefs[2,1]),
  " (p = ", p_txt, "), N=",n_model
)
...
modelsummary<-summary(lm_robust(BLM~CoParty_control,data7_attentive))
n_model <- nobs(lm_robust(BLM~CoParty_control,data7_attentive)) #94
coefs <- modelsummary$coefficients
p_exact <- coefs["CoParty_control", "Pr(>|t|)"]   #
...
ggplot(main_fx, aes(x = outcome, y = estimate, color = Sample, shape = Sample)) +
  ...
  annotate( #choice-full
    "text",
    x = 0.5, y = -0.15,
    label = stat_txt_choicefull,
    hjust = 0, vjust = 1,
    color = "firebrick2", size = 2.5
  ) +
    annotate( #choice-attentive
    "text",
    x = 1, y = 0.2,
    label = stat_txt_choiceattentive,
    hjust = 0, vjust = 1,
    color = "dodgerblue2", size = 2.5
  ) +
  annotate( #blm-full
    "text",
    x = 1.4, y = 0.12,
    label = stat_txt_blmfull,
    hjust = 0, vjust = 1,
    color = "firebrick2", size = 2.5
  ) +
  annotate( #blm-attentive
    "text",
    x = 2, y = -0.1,
    label = stat_txt_blmattentive,
    hjust = 0, vjust = 1,
    color = "dodgerblue2", size = 2.5
  ) 
...
#models
ate3D <- miceadds::glm.cluster( data=long_data3D, formula=describeORfeel ~ PraiseEmpathy, cluster="ID", family=binomial(link="logit"))
summary(ate3D) 
ate3R <- miceadds::glm.cluster( data=long_data3R, formula=describeORfeel ~ PraiseEmpathy, cluster="ID", family=binomial(link="logit"))
summary(ate3R) 
ate3I <- miceadds::glm.cluster( data=long_data3I, formula=describeORfeel ~ PraiseEmpathy, cluster="ID", family=binomial(link="logit"))
summary(ate3I) 
#study 5/6
...
#keep annotation of text
#In order, Dem-3-5-Pool, Rep-3-5-Pool, Indep-3-5-Pool 
N<- c(1455,1214,2669,1890,813,2703,1140,333,783)
n<- c(97,408,505,126,274,400,76,111,141)

coefs <- c(coef(ate3D)[2],coef(ate5D)[2],coef(atetotD)[2],
           coef(ate3R)[2],coef(ate5R)[2],coef(atetotR)[2],
           coef(ate3I)[2],coef(ate5I)[2],coef(atetotI)[2])
p_exact <- c(summary(ate3D)[2,4],summary(ate5D)[2,4],summary(atetotD)[2,4],
           summary(ate3R)[2,4],summary(ate5R)[2,4],summary(atetotR)[2,4],
           summary(ate3I)[2,4],summary(ate5I)[2,4],summary(atetotI)[2,4])

#text to annotate later
p_txt <- ifelse(p_exact < 1e-3,
                format(p_exact, scientific = TRUE, digits = 2),
                formatC(p_exact, format = "f", digits = 4))
stat_txt<-rep(NA,length(p_txt))
for(i in 1:length(p_txt)){
  stat_txt[i]<- paste0("Coef Peer Praise:\n ", sprintf("%.3f", coefs[i]),
  " (p = ", p_txt[i], "), N=",N[i],",n=",n[i])
}

#set up color-annotate
vir_col <- viridis(3, option = "D")
#plot
...
plot_subparty<-ggplot(plot_data,aes(y=estimate,x=type,group=group)) +
  ...
  scale_color_viridis(#alpha=1,
                      begin=1,end=0,discrete=T,option="D",guide = guide_legend(override.aes = list(size = 0.75,alpha = 1) )) +
  xlab("") + ylab("Subgroup treatment effect estimate") + ylim(-0.4,0.65) + ggtitle("by Party") +
  coord_flip() + theme_bw() + 
  theme(legend.title = element_blank()
        ,axis.text.y=element_text(size=12,face="bold")) +
  annotate("text", x = 0.9, y = 0, label = stat_txt[1], #Dem-3
    hjust = 0, vjust = 1, color = vir_col[3], size = 2) +
  annotate("text", x = 1.12, y = -0.35, label = stat_txt[2], #Dem-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) +
  annotate("text", x = 1.2, y = 0, label = stat_txt[3], #Dem-Pool
    hjust = 0, vjust = 1, color = vir_col[1], size = 2) +  
  annotate("text", x = 1.9, y = 0, label = stat_txt[4], #R-3
    hjust = 0, vjust = 1, color = vir_col[3], size = 2) + 
  annotate("text", x = 2.12, y = -0.35, label = stat_txt[5], #R-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) + 
  annotate("text", x = 2.2, y = 0, label = stat_txt[6], #R-Pool
    hjust = 0, vjust = 1, color = vir_col[1], size = 2) + 
  annotate("text", x = 2.9, y = 0, label = stat_txt[7], #I-3
    hjust = 0, vjust = 1, color = vir_col[3], size = 2) + 
  annotate("text", x = 3.12, y = -0.4, label = stat_txt[8], #I-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) + 
  annotate("text", x = 3.2, y = 0, label = stat_txt[9], #I-Pool
    hjust = 0, vjust = 1, color = vir_col[1], size = 2)  
plot_subparty
...
estimate<-se<-p<-matrix(NA,nrow=7,ncol=3) #rows are approval levels, cols are data source
...
plot_data$group<-factor(plot_data$group,levels = c("Study 3","Study 5A & 5B","Pooled"))
#annotations
  #sort by annotation level:
plot_data_sorted <- plot_data %>%
  mutate(
    # Explicit plot ordering of approval
    type = factor(
      type,
      levels = c(
    ...
    # Explicit plot ordering of study
    group = factor(
      group,
      levels = c("Study 3", "Study 5A & 5B", "Pooled"),
      ordered = TRUE
    )
  ) %>%
  arrange(type, group)

p_txt <- ifelse(plot_data_sorted$p < 1e-3,
                format(plot_data_sorted$p, scientific = TRUE, digits = 2),
                formatC(plot_data_sorted$p, format = "f", digits = 4))
stat_txt<-rep(NA,length(p_txt))
for(i in 1:length(p_txt)){
  stat_txt[i]<- paste0("Coef Peer Praise:\n ", sprintf("%.3f", plot_data_sorted$estimate[i]),
  " (p = ", p_txt[i], ")")
}

plot_subapproveTrump<-ggplot(plot_data,aes(y=estimate,x=type,group=group)) +
  geom_pointrange(aes(ymin = lower, ymax = upper, color = group), position = position_dodge(width = 0.2), fatten = 1.5, size=0.9,linewidth=0.9) +
  geom_pointrange(aes(ymin = lower90, ymax = upper90, color = group), position = position_dodge(width = 0.2), fatten = 1.5, size=2,linewidth=2) +
  geom_hline(yintercept=0,color="gray75",linetype="dashed") +
  scale_color_viridis(begin=1,end=0,discrete=T,option="D",guide = guide_legend(override.aes = list(size = 0.75,alpha = 1) )) +
  xlab("") + ylab("Subgroup treatment effect estimate") + ylim(-1.1,1.1) + ggtitle("by Trump approval") +
  coord_flip() + theme_bw() + 
  theme(legend.title = element_blank(),axis.text.y=element_text(size=12,face="bold")) +
  annotate("text", x = 0.8, y = -1, label = stat_txt[1], #7-P
    hjust = 0, vjust = 1, color = vir_col[3], size = 2)  +
  annotate("text", x = 1, y = -1, label = stat_txt[2], #7-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2)  +
  annotate("text", x = 1.2, y = -1, label = stat_txt[3], #7-3
    hjust = 0, vjust = 1, color = vir_col[1], size = 2)  +
  annotate("text", x = 1.8, y = -1, label = stat_txt[4], #6-P
    hjust = 0, vjust = 1, color = vir_col[3], size = 2)  +
  annotate("text", x = 2, y = -1, label = stat_txt[5], #6-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2)  +
  annotate("text", x = 2.2, y = -1, label = stat_txt[6], #6-3
    hjust = 0, vjust = 1, color = vir_col[1], size = 2)  +
  annotate("text", x = 2.8, y = -1, label = stat_txt[7], #5-P
    hjust = 0, vjust = 1, color = vir_col[3], size = 2)  +
  annotate("text", x = 3, y = -1, label = stat_txt[8], #5-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2)  +
  annotate("text", x = 3.2, y = -1, label = stat_txt[9], #5-3
    hjust = 0, vjust = 1, color = vir_col[1], size = 2)  +
  annotate("text", x = 3.8, y = -1, label = stat_txt[10], #4-P
    hjust = 0, vjust = 1, color = vir_col[3], size = 2)  +
  annotate("text", x = 4, y = -1, label = stat_txt[11], #4-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2)  +
  annotate("text", x = 4.2, y = -1, label = stat_txt[12], #4-3
    hjust = 0, vjust = 1, color = vir_col[1], size = 2)  +
  annotate("text", x = 4.8, y = -1, label = stat_txt[13], #3-P
    hjust = 0, vjust = 1, color = vir_col[3], size = 2)  +
  annotate("text", x = 5, y = -1, label = stat_txt[14], #3-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2)  +
  annotate("text", x = 5.2, y = -1, label = stat_txt[15], #3-3
    hjust = 0, vjust = 1, color = vir_col[1], size = 2)  +
  annotate("text", x = 5.8, y = -1, label = stat_txt[16], #2-P
    hjust = 0, vjust = 1, color = vir_col[3], size = 2)  +
  annotate("text", x = 6, y = -1, label = stat_txt[17], #2-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2)  +
  annotate("text", x = 6.2, y = -1, label = stat_txt[18], #2-3
    hjust = 0, vjust = 1, color = vir_col[1], size = 2)  +
  annotate("text", x = 6.8, y = -1, label = stat_txt[19], #1-P
    hjust = 0, vjust = 1, color = vir_col[3], size = 2)  +
  annotate("text", x = 7, y = -1, label = stat_txt[20], #1-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2)  +
  annotate("text", x = 7.2, y = -1, label = stat_txt[21], #1-3
    hjust = 0, vjust = 1, color = vir_col[1], size = 2)  
...
estimate<-se<-p<-matrix(NA,nrow=7,ncol=1) #rows are approval levels, cols are data source
...
#text to annotate later
p_txt <- ifelse(p[,1] < 1e-3,
                format(p[,1], scientific = TRUE, digits = 2),
                formatC(p[,1], format = "f", digits = 4))
stat_txt<-rep(NA,length(p_txt))
for(i in 1:length(p_txt)){
  stat_txt[i]<- paste0("Coef Peer Praise:\n ", sprintf("%.3f", estimate[i,1]),
  " (p = ", p_txt[i], ")")
}
...
plot_subapproveBiden<-ggplot(plot_data,aes(y=estimate,x=type,group=group)) +
  ...
  theme(legend.title = element_blank()
        ,axis.text.y=element_text(size=12,face="bold")) +
  annotate("text", x = 0.9, y = 0, label = stat_txt[1], #1
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) +
  annotate("text", x = 1.9, y = 0, label = stat_txt[2], #2
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) +
  annotate("text", x = 2.9, y = 0, label = stat_txt[3], #3
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) +
  annotate("text", x = 3.9, y = 0, label = stat_txt[4], #4
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) +
  annotate("text", x = 4.9, y = 0, label = stat_txt[5], #5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) +
  annotate("text", x = 5.9, y = 0, label = stat_txt[6], #6
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) +
  annotate("text", x = 6.9, y = 0, label = stat_txt[7], #7
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) 
...
#100k or more
ate3100k <- miceadds::glm.cluster( data=long_data3100k, formula=describeORfeel ~ PraiseEmpathy, cluster="ID", family=binomial(link="logit"))
summary(ate3100k) 
  #n
...
plot_data<-data.frame(group=rep(c("Study 3","Study 5A & 5B","Pooled"),each=4)
           ,type=rep(c("100k or more", "75k to less than 100k", "50k to less than 75k", "25k to less than 50k"),times=3),estimate=c(coef(ate3100k)[2],coef(ate375k)[2],coef(ate350k)[2], coef(ate325k)[2],coef(ate5100k)[2],coef(ate575k)[2],coef(ate550k)[2],coef(ate525k)[2],coef(atepool100k)[2],coef(atepool75k)[2],coef(atepool50k)[2],coef(atepool25k)[2])
           ...
           ,p=c(summary(ate3100k)[2,4],summary(ate375k)[2,4],
                 summary(ate350k)[2,4],summary(ate325k)[2,4],
                 summary(ate5100k)[2,4],summary(ate575k)[2,4],
                 summary(ate550k)[2,4], summary(ate525k)[2,4],
                 summary(atepool100k)[2,4],summary(atepool75k)[2,4],
                 summary(atepool50k)[2,4],summary(atepool25k)[2,4] ))
...
plot_data$group<-factor(plot_data$group,levels = c("Study 3","Study 5A & 5B","Pooled"))
#annotate, first arrange and then annotate in order
plot_data_sorted <- plot_data %>%
  mutate(
    # Explicit income ordering (highest to lowest)
    type = factor(
      type,
      levels = c(
        "100k or more",
...
plot_subincome<-ggplot(plot_data,aes(y=estimate,x=type,group=group)) +
  ...
  theme(legend.title = element_blank()
        ,axis.text.y=element_text(size=12,face="bold")) +
  annotate("text", x = 0.9, y = -0.51, label = stat_txt[1], #1-3
    hjust = 0, vjust = 1, color = vir_col[3], size = 2) +
  annotate("text", x = 1.1, y = -0.51, label = stat_txt[2], #1-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) +
  annotate("text", x = 1.3, y = -0.51, label = stat_txt[3], #1-P
    hjust = 0, vjust = 1, color = vir_col[1], size = 2) +
  annotate("text", x = 1.9, y = -0.51, label = stat_txt[4], #2-3
    hjust = 0, vjust = 1, color = vir_col[3], size = 2) +
  annotate("text", x = 2.1, y = -0.51, label = stat_txt[5], #2-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) +
  annotate("text", x = 2.3, y = -0.51, label = stat_txt[6], #2-p
    hjust = 0, vjust = 1, color = vir_col[1], size = 2) +
  annotate("text", x = 2.9, y = -0.51, label = stat_txt[7], #3-3
    hjust = 0, vjust = 1, color = vir_col[3], size = 2) +
  annotate("text", x = 3.1, y = -0.51, label = stat_txt[8], #3-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) +
  annotate("text", x = 3.3, y = -0.51, label = stat_txt[9], #3-p
    hjust = 0, vjust = 1, color = vir_col[1], size = 2) +
  annotate("text", x = 3.9, y = -0.51, label = stat_txt[10], #4-3
    hjust = 0, vjust = 1, color = vir_col[3], size = 2) +
  annotate("text", x = 4.1, y = -0.51, label = stat_txt[11], #4-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) +
  annotate("text", x = 4.3, y = -0.51, label = stat_txt[12], #4-p
    hjust = 0, vjust = 1, color = vir_col[1], size = 2) 
plot_subincome
#ate_subincome.pdf 
...
plot_data<-data.frame(group=rep(c("Study 3","Study 5A & 5B","Pooled"),each=3)
           ...
           ,p=c(summary(ate3Black)[2,4],summary(ate3White)[2,4],summary(ate3Other)[2,4]
                 ,summary(ate5Black)[2,4],summary(ate5White)[2,4],summary(ate5Other)[2,4]
                 ,summary(atetotBlack)[2,4],summary(atetotWhite)[2,4],summary(atetotOther)[2,4]))
plot_data$lower=plot_data$estimate-qnorm(0.975)*plot_data$se
plot_data$upper=plot_data$estimate+qnorm(0.975)*plot_data$se
plot_data$lower90=plot_data$estimate-qnorm(0.95)*plot_data$se
plot_data$upper90=plot_data$estimate+qnorm(0.95)*plot_data$se
...
plot_subrace<-ggplot(plot_data,aes(y=estimate,x=type,group=group)) +
  ...
  theme(legend.title = element_blank()
        ,axis.text.y=element_text(size=12,face="bold")) +
  annotate("text", x = 0.9, y = -0.51, label = stat_txt[1], #b-3
    hjust = 0, vjust = 1, color = vir_col[3], size = 2) +
  annotate("text", x = 1.1, y = -0.51, label = stat_txt[2], #b-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) +
  annotate("text", x = 1.3, y = -0.51, label = stat_txt[3], #b-P
    hjust = 0, vjust = 1, color = vir_col[1], size = 2) +
  annotate("text", x = 1.9, y = -0.51, label = stat_txt[4], #w-3
    hjust = 0, vjust = 1, color = vir_col[3], size = 2) +
  annotate("text", x = 2.1, y = -0.51, label = stat_txt[5], #w-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) +
  annotate("text", x = 2.3, y = -0.51, label = stat_txt[6], #w-p
    hjust = 0, vjust = 1, color = vir_col[1], size = 2) +
  annotate("text", x = 2.9, y = -0.51, label = stat_txt[7], #o-3
    hjust = 0, vjust = 1, color = vir_col[3], size = 2) +
  annotate("text", x = 3.1, y = -0.51, label = stat_txt[8], #o-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) +
  annotate("text", x = 3.3, y = -0.51, label = stat_txt[9], #o-p
    hjust = 0, vjust = 1, color = vir_col[1], size = 2) 
...
estimate<-se<-p<-matrix(NA,nrow=length(educ_levels),ncol=3) #rows are educ levels, cols are data source
...
plot_subeduc<-ggplot(plot_data,aes(y=estimate,x=type,group=group)) +
  ...
  theme(legend.title = element_blank()
        ,axis.text.y=element_text(size=12,face="bold")) +
  annotate("text", x = 0.9, y = -0.51, label = stat_txt[1], #hs-3
    hjust = 0, vjust = 1, color = vir_col[3], size = 2) +
  annotate("text", x = 1.1, y = -0.51, label = stat_txt[2], #hs-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) +
  annotate("text", x = 1.3, y = -0.51, label = stat_txt[3], #hs-P
    hjust = 0, vjust = 1, color = vir_col[1], size = 2) +
  annotate("text", x = 1.9, y = -0.51, label = stat_txt[4], #c-3
    hjust = 0, vjust = 1, color = vir_col[3], size = 2) +
  annotate("text", x = 2.1, y = -0.51, label = stat_txt[5], #c-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) +
  annotate("text", x = 2.3, y = -0.51, label = stat_txt[6], #c-p
    hjust = 0, vjust = 1, color = vir_col[1], size = 2) +
  annotate("text", x = 2.9, y = -0.51, label = stat_txt[7], #p-3
    hjust = 0, vjust = 1, color = vir_col[3], size = 2) +
  annotate("text", x = 3.1, y = -0.51, label = stat_txt[8], #p-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) +
  annotate("text", x = 3.3, y = -0.51, label = stat_txt[9], #p-p
    hjust = 0, vjust = 1, color = vir_col[1], size = 2) 
plot_subeduc
#ate_subeduc.pdf 6x6
...
plot_data<-data.frame(group=rep(c("Study 3","Study 5A & 5B","Pooled"),each=2)
           ...
           ,p=c(summary(ate3F)[2,4],summary(ate3M)[2,4]
                 ,summary(ate5F)[2,4],summary(ate5M)[2,4]
                 ,summary(atetotF)[2,4],summary(atetotM)[2,4]))
plot_data$lower=plot_data$estimate-qnorm(0.975)*plot_data$se
plot_data$upper=plot_data$estimate+qnorm(0.975)*plot_data$se
plot_data$lower90=plot_data$estimate-qnorm(0.95)*plot_data$se
plot_data$upper90=plot_data$estimate+qnorm(0.95)*plot_data$se
...
plot_subsex<-ggplot(plot_data,aes(y=estimate,x=type,group=group)) +
  ...
  scale_color_viridis(#alpha=1,
                      begin=1,end=0.0,discrete=T,option="D",guide = guide_legend(override.aes = list(size = 0.75,
                                                                    alpha = 1) )) +
  xlab("") + ylab("Subgroup treatment effect estimate") + ylim(-0.3,0.8) + ggtitle("by Sex") +
  coord_flip() + theme_bw() + 
  theme(legend.title = element_blank()
        ,axis.text.y=element_text(size=12,face="bold")) +
  annotate("text", x = 0.9, y = -0.2, label = stat_txt[1], #hs-3
    hjust = 0, vjust = 1, color = vir_col[3], size = 2) +
  annotate("text", x = 1.1, y = -0.2, label = stat_txt[2], #hs-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) +
  annotate("text", x = 1.3, y = -0.2, label = stat_txt[3], #hs-P
    hjust = 0, vjust = 1, color = vir_col[1], size = 2) +
  annotate("text", x = 1.9, y = -0.2, label = stat_txt[4], #c-3
    hjust = 0, vjust = 1, color = vir_col[3], size = 2) +
  annotate("text", x = 2.1, y = -0.2, label = stat_txt[5], #c-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) +
  annotate("text", x = 2.3, y = -0.2, label = stat_txt[6], #c-p
    hjust = 0, vjust = 1, color = vir_col[1], size = 2) 
...
plot_data<-data.frame(group=rep(c("Study 3","Study 5A & 5B","Pooled"),each=3)
           ...
           ,p=c(summary(ate3lowemp)[2,4],summary(ate3medemp)[2,4],summary(ate3highemp)[2,4]
                 ,summary(ate5lowemp)[2,4],summary(ate5medemp)[2,4],summary(ate5highemp)[2,4]
                 ,summary(atetotlowemp)[2,4],summary(atetotmedemp)[2,4],summary(atetothighemp)[2,4]))
plot_data$lower=plot_data$estimate-qnorm(0.975)*plot_data$se
plot_data$upper=plot_data$estimate+qnorm(0.975)*plot_data$se
plot_data$lower90=plot_data$estimate-qnorm(0.95)*plot_data$se
plot_data$upper90=plot_data$estimate+qnorm(0.95)*plot_data$se
...
plot_data_sorted <- plot_data %>%
  mutate(
    # Explicit baseline empathy ordering
    type = factor(
      type,
      levels = c(
        "Low",
...
plot_subbaseempathy<-ggplot(plot_data,aes(y=estimate,x=type,group=group)) +
  ...
  theme(legend.title = element_blank()
        ,axis.text.y=element_text(size=12,face="bold"))  +
  annotate("text", x = 0.9, y = -0.2, label = stat_txt[1], #l-3
    hjust = 0, vjust = 1, color = vir_col[3], size = 2) +
  annotate("text", x = 1.1, y = -0.2, label = stat_txt[2], #l-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) +
  annotate("text", x = 1.3, y = -0.2, label = stat_txt[3], #l-P
    hjust = 0, vjust = 1, color = vir_col[1], size = 2) +
  annotate("text", x = 1.9, y = -0.2, label = stat_txt[4], #m-3
    hjust = 0, vjust = 1, color = vir_col[3], size = 2) +
  annotate("text", x = 2.1, y = -0.2, label = stat_txt[5], #m-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) +
  annotate("text", x = 2.3, y = -0.2, label = stat_txt[6], #m-p
    hjust = 0, vjust = 1, color = vir_col[1], size = 2) +
  annotate("text", x = 2.9, y = -0.2, label = stat_txt[7], #h-3
    hjust = 0, vjust = 1, color = vir_col[3], size = 2) +
  annotate("text", x = 3.1, y = -0.2, label = stat_txt[8], #h-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) +
  annotate("text", x = 3.3, y = -0.2, label = stat_txt[9], #h-p
    hjust = 0, vjust = 1, color = vir_col[1], size = 2) 
...
plot_data<-data.frame(group=rep(c("Study 5A","Study 5B","Pooled"),each=3)
           ...
           ,p=c(summary(ate_attentive5)[2,4],summary(ate_halfattentive5)[2,4],summary(ate_unattentive5)[2,4]
                 ,summary(ate_attentive6)[2,4],summary(ate_halfattentive6)[2,4],summary(ate_unattentive6)[2,4]
                 ,summary(ate_attentivepool)[2,4],summary(ate_halfattentivepool)[2,4],summary(ate_unattentivepool)[2,4]))
plot_data$lower=plot_data$estimate-qnorm(0.975)*plot_data$se
plot_data$upper=plot_data$estimate+qnorm(0.975)*plot_data$se
plot_data$lower90=plot_data$estimate-qnorm(0.95)*plot_data$se
plot_data$upper90=plot_data$estimate+qnorm(0.95)*plot_data$se
...
plot_data_sorted <- plot_data %>%
  mutate(
    ...
    group = factor(
      group,
      levels = c("Study 5A", "Study 5B", "Pooled"),
      ordered = TRUE
    )
  ) %>%
  arrange(type, group)
...
plot_subattentive<-ggplot(plot_data,aes(y=estimate,x=type,group=group)) +
  ...
  theme(legend.title = element_blank()
        ,axis.text.y=element_text(size=12,face="bold")) +
  annotate("text", x = 0.9, y = -1, label = stat_txt[1], #l-3
    hjust = 0, vjust = 1, color = vir_col[3], size = 2) +
  annotate("text", x = 1.1, y = -1, label = stat_txt[2], #l-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) +
  annotate("text", x = 1.3, y = -1, label = stat_txt[3], #l-P
    hjust = 0, vjust = 1, color = vir_col[1], size = 2) +
  annotate("text", x = 1.9, y = -1, label = stat_txt[4], #m-3
    hjust = 0, vjust = 1, color = vir_col[3], size = 2) +
  annotate("text", x = 2.1, y = -1, label = stat_txt[5], #m-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) +
  annotate("text", x = 2.3, y = -1, label = stat_txt[6], #m-p
    hjust = 0, vjust = 1, color = vir_col[1], size = 2) +
  annotate("text", x = 2.9, y = -1, label = stat_txt[7], #h-3
    hjust = 0, vjust = 1, color = vir_col[3], size = 2) +
  annotate("text", x = 3.1, y = -1, label = stat_txt[8], #h-5
    hjust = 0, vjust = 1, color = vir_col[2], size = 2) +
  annotate("text", x = 3.3, y = -1, label = stat_txt[9], #h-p
    hjust = 0, vjust = 1, color = vir_col[1], size = 2) 
...
plot_data <-data.frame(Trial=factor(paste("Trial",1:20, sep=" "),levels = paste("Trial",1:20, sep=" "))
                       ...
                       ,ACME=NA,ACME_p=NA,ACME_low=NA,ACME_high=NA
                       ,TE=NA,TE_p=NA,TE_low=NA,TE_high=NA,N=NA)
...
plot_data_long<-data.frame(Trial=rep(plot_data$Trial,2)
                           ...
                           ,p=c(plot_data$ACME_p,plot_data$TE_p)
...
#annotate, first arrange and then annotate in order
plot_data_long_sorted <- plot_data_long %>%
  mutate(
    # Extract numeric trial index
    trial_num = as.integer(str_extract(Trial, "\\d+")),
    
    # Explicitly order Estimand
    Estimand = factor(
      Estimand,
      levels = c("ACME", "TE"),
      ordered = TRUE
  ...
  arrange(trial_num, Estimand)

#text to annotate later
p_txt <- ifelse(plot_data_long_sorted$p < 1e-3,
                format(plot_data_long_sorted$p, scientific = TRUE, digits = 2),
                formatC(plot_data_long_sorted$p, format = "f", digits = 4))
stat_txt<-rep(NA,length(p_txt))
for(i in 1:length(p_txt)){
  stat_txt[i]<- paste0("Est: ", sprintf("%.3f", plot_data_long_sorted$Estimate[i]),
  "\n(p = ", p_txt[i], ")")
}


x_loc<-seq(0.5,by=1,length.out=20)
 ...
 ggplot(plot_data_long,aes(x=Trial, y=Estimate)) +
   ...
   labs(x = "Cumulative trials",
       y = "Estimated Effect Size") + ylim(-0.1,0.2) +
  theme(text = element_text(size = 10, family = "Times"),
        legend.key=element_blank(),
        panel.grid.major = element_blank(), 
        axis.text.x = element_text(size = 10),
        ...
        panel.background = element_blank(), 
        axis.line = element_line(colour = "black")) +
  annotate("text", x = x_loc[1], y = -0.1, label = stat_txt[1], #1-a
    hjust = 0, vjust = 1, color = "firebrick3", size = 2) +
  annotate("text", x = x_loc[1]+0.1, y = -0.09, label = stat_txt[2], #1-t
    hjust = 0, vjust = 1, color = "darkblue", size = 2) +
   
  annotate("text", x = x_loc[2]+0.1, y = 0.15, label = stat_txt[3], #2-a
    hjust = 0, vjust = 1, color = "firebrick3", size = 2) +
  annotate("text", x = x_loc[2]+0.1, y = 0.16, label = stat_txt[4], #2-t
    hjust = 0, vjust = 1, color = "darkblue", size = 2) +
   
  annotate("text", x = x_loc[3], y = -0.1, label = stat_txt[5], #3-a
    hjust = 0, vjust = 0.9, color = "firebrick3", size = 2) +
  annotate("text", x = x_loc[3]+0.1, y = -0.09, label = stat_txt[6], #3-t
    hjust = 0, vjust = 1, color = "darkblue", size = 2) +
   
  annotate("text", x = x_loc[4], y = 0.15, label = stat_txt[7], #4-a
    hjust = 0, vjust = 1, color = "firebrick3", size = 2) +
  annotate("text", x = x_loc[4]+0.1, y = 0.16, label = stat_txt[8], #4-t
    hjust = 0, vjust = 1, color = "darkblue", size = 2) +
   
  annotate("text", x = x_loc[5], y = -0.1, label = stat_txt[9], #5-a
    hjust = 0, vjust = 0.9, color = "firebrick3", size = 2) +
  annotate("text", x = x_loc[5]+0.1, y = -0.09, label = stat_txt[10], #5-t
    hjust = 0, vjust = 1, color = "darkblue", size = 2) +
   
  annotate("text", x = x_loc[6], y = 0.15, label = stat_txt[11], #6-a
    hjust = 0, vjust = 1, color = "firebrick3", size = 2) +
  annotate("text", x = x_loc[6]+0.1, y = 0.16, label = stat_txt[12], #6-t
    hjust = 0, vjust = 1, color = "darkblue", size = 2) +

  annotate("text", x = x_loc[7], y = -0.1, label = stat_txt[13], #7-a
    hjust = 0, vjust = 0.9, color = "firebrick3", size = 2) +
  annotate("text", x = x_loc[7]+0.1, y = -0.09, label = stat_txt[14], #7-t
    hjust = 0, vjust = 1, color = "darkblue", size = 2) +
   
  annotate("text", x = x_loc[8], y = 0.15, label = stat_txt[15], #8-a
    hjust = 0, vjust = 1, color = "firebrick3", size = 2) +
  annotate("text", x = x_loc[8]+0.1, y = 0.16, label = stat_txt[16], #8-t
    hjust = 0, vjust = 1, color = "darkblue", size = 2) +
   
  annotate("text", x = x_loc[9], y = -0.1, label = stat_txt[17], #9-a
    hjust = 0, vjust = 0.9, color = "firebrick3", size = 2) +
  annotate("text", x = x_loc[9]+0.1, y = -0.09, label = stat_txt[18], #9-t
    hjust = 0, vjust = 1, color = "darkblue", size = 2) +
   
  annotate("text", x = x_loc[10], y = 0.15, label = stat_txt[19], #10-a
    hjust = 0, vjust = 1, color = "firebrick3", size = 2) +
  annotate("text", x = x_loc[10]+0.1, y = 0.16, label = stat_txt[20], #10-t
    hjust = 0, vjust = 1, color = "darkblue", size = 2) +

  annotate("text", x = x_loc[11], y = -0.1, label = stat_txt[21], #11-a
    hjust = 0, vjust = 0.9, color = "firebrick3", size = 2) +
  annotate("text", x = x_loc[11]+0.1, y = -0.09, label = stat_txt[22], #11-t
    hjust = 0, vjust = 1, color = "darkblue", size = 2) +
   
  annotate("text", x = x_loc[12], y = 0.15, label = stat_txt[23], #12-a
    hjust = 0, vjust = 1, color = "firebrick3", size = 2) +
  annotate("text", x = x_loc[12]+0.1, y = 0.16, label = stat_txt[24], #12-t
    hjust = 0, vjust = 1, color = "darkblue", size = 2) +
   
  annotate("text", x = x_loc[13], y = -0.1, label = stat_txt[25], #13-a
    hjust = 0, vjust = 0.9, color = "firebrick3", size = 2) +
  annotate("text", x = x_loc[13]+0.1, y = -0.09, label = stat_txt[26], #13-t
    hjust = 0, vjust = 1, color = "darkblue", size = 2) +
   
  annotate("text", x = x_loc[14], y = 0.15, label = stat_txt[27], #14-a
    hjust = 0, vjust = 1, color = "firebrick3", size = 2) +
  annotate("text", x = x_loc[14]+0.1, y = 0.16, label = stat_txt[28], #14-t
    hjust = 0, vjust = 1, color = "darkblue", size = 2) +
   
  annotate("text", x = x_loc[15], y = -0.1, label = stat_txt[29], #15-a
    hjust = 0, vjust = 0.9, color = "firebrick3", size = 2) +
  annotate("text", x = x_loc[15]+0.1, y = -0.09, label = stat_txt[30], #15-t
    hjust = 0, vjust = 1, color = "darkblue", size = 2) +
   
  annotate("text", x = x_loc[16], y = 0.15, label = stat_txt[31], #16-a
    hjust = 0, vjust = 1, color = "firebrick3", size = 2) +
  annotate("text", x = x_loc[16]+0.1, y = 0.16, label = stat_txt[32], #16-t
    hjust = 0, vjust = 1, color = "darkblue", size = 2) +
   
  annotate("text", x = x_loc[17], y = -0.1, label = stat_txt[33], #17-a
    hjust = 0, vjust = 0.9, color = "firebrick3", size = 2) +
  annotate("text", x = x_loc[17]+0.1, y = -0.09, label = stat_txt[34], #17-t
    hjust = 0, vjust = 1, color = "darkblue", size = 2) +
   
  annotate("text", x = x_loc[18], y = 0.15, label = stat_txt[35], #18-a
    hjust = 0, vjust = 1, color = "firebrick3", size = 2) +
  annotate("text", x = x_loc[18]+0.1, y = 0.16, label = stat_txt[36], #18-t
    hjust = 0, vjust = 1, color = "darkblue", size = 2) +
   
  annotate("text", x = x_loc[19], y = -0.1, label = stat_txt[37], #19-a
    hjust = 0, vjust = 0.9, color = "firebrick3", size = 2) +
  annotate("text", x = x_loc[19]+0.1, y = -0.09, label = stat_txt[38], #19-t
    hjust = 0, vjust = 1, color = "darkblue", size = 2) +
   
  annotate("text", x = x_loc[20], y = 0.15, label = stat_txt[39], #20-a
    hjust = 0, vjust = 1, color = "firebrick3", size = 2) +
  annotate("text", x = x_loc[20]+0.1, y = 0.16, label = stat_txt[40], #20-t
    hjust = 0, vjust = 1, color = "darkblue", size = 2) 
#cum_acme_te.pdf 
 
ggsave(file=here::here("Figures","cum_acme_te.pdf"),width=11,height=8)
